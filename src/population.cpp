...
bool Population::epoch(int generation) {
	...
	*/
	...
	for(curspecies=sorted_species.begin();curspecies!=sorted_species.end();++curspecies) {
		...
		std::cout<<"Orig fit of Spec["<<(*curspecies)->id<<"]\t(Size "<<(*curspecies)->organisms.size()<<"): "<<(*((*curspecies)->organisms).begin())->orig_fitness<<"\tLast improved "<<((*curspecies)->age-(*curspecies)->age_of_last_improvement)<<std::endl;
	}
